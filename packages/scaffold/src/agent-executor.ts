...
export async function initializeToolAgentExecutor(options: CreateOpenAIFunctionsAgentParams ): Promise<Runnable> {
    
    const asAgentFinish = (x?: AgentAction | AgentFinish  ) => 
        (x && "returnValues" in x) ? x as AgentFinish : undefined;

    const asAgentAction = (x?: AgentAction | AgentFinish  ) => 
