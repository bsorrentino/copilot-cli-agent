...
/**
 * An interface representing the state of a schema generator.
 */
...
export async function schemaGeneratorAgentExecutor({ llm }: Pick<CreateOpenAIFunctionsAgentParams, "llm">): Promise<Runnable> {
    ...
    const parseContent = async (message: BaseMessage) => {

        const output = await outputParser.parse(message.content as string);
    ...
    const runAskForInput = async (data: GeneratorState) => {
        const { messages } = data
    ...
    const runAgentGenerator = async (data: GeneratorState, config?: RunnableConfig): Promise<Pick<GeneratorState, 'messages' | 'result'>> => {
    ...
    const humanValidator = async (data: GeneratorState) => {
        const { result } = data;

        if (result === undefined) throw new Error("the result is undefined");

        const confirm = await askForconfirmSchema(result);

        if (confirm) return { input: 'YES' };
    ...
    const shouldContinue = (data: GeneratorState) => {
        const { input } = data;
